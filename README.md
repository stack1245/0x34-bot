...
일정 추가, 생성, 수정, 삭제가 끝나면 봇은 일정 채널에 새 메시지를 계속 보내지 않고 하나의 일정 대시보드 메시지를 갱신합니다. 대시보드의 채널 ID와 메시지 ID는 SQLite의 `dashboard_state` 테이블에 저장되며, 저장된 메시지가 삭제되면 다음 갱신 때 새 대시보드를 만들고 위치를 다시 저장합니다. 최초 대시보드 생성 채널은 `SCHEDULE_CHANNEL_ID`가 우선이고, 없으면 서버 기본 시스템 채널을 사용합니다.
