...
- `/일정추가`: Modal로 제목, 날짜/시간, 내용을 받아 일정을 저장하고 서버 이벤트 생성을 시도합니다. 날짜/시간은 `내일 오후 3시`, `7월 13일 ~ 14일` 같은 자연어 입력도 Gemini로 해석하며, AI API 지연 시 `YYYY-MM-DD HH:MM` 또는 `YYYY-MM-DD` 형식은 수동 파싱으로 저장합니다.
