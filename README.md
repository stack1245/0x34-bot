...
`/모집생성`과 모집 마감 시 일정 자동 추출은 Google AI Studio에서 발급한 Gemini API 키가 필요합니다. 로컬 `.env` 또는 Railway Variables에 아래 값을 추가하세요.
...
모집 글이 생성되면 같은 채널에 비공개 워크스페이스 스레드를 만들고 작성자를 즉시 초대합니다. `[참가]` 버튼을 누른 팀원은 자동으로 해당 비공개 스레드에 추가됩니다. 작성자가 `[모집 마감]` 버튼을 누르면 봇이 모집 Embed에서 일정 날짜와 장소를 추출해 `schedules`에 자동 등록하고, `SCHEDULE_CHANNEL_ID`가 있으면 일정 채널에 공지합니다. 날짜를 자동 추출하지 못하면 "일정 날짜를 자동 추출하지 못했습니다. 수동으로 등록해 주세요."라고 안내합니다. 봇에는 채널 기준 `Create Private Threads`, `Send Messages in Threads`, 필요 시 `Manage Threads` 권한이 있어야 합니다.
