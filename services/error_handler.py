...
class BotErrorHandler:
    """Centralized graceful-degradation handler for commands and events."""
    ...
    async def handle_app_command_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError) -> None:
        ...
        if isinstance(error, app_commands.CommandOnCooldown):
            message = f"잠시 후 다시 시도해 주세요. 남은 시간: {error.retry_after:.1f}초"
            ...
            message = "대상 메시지 또는 채널을 찾을 수 없습니다. 상태를 새로고침해 주세요."
        elif isinstance(original, discord.HTTPException):
            message = "Discord API가 일시적으로 실패했습니다. 잠시 후 다시 시도해 주세요."
        else:
